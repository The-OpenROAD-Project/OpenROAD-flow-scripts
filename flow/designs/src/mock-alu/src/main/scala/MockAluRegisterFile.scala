...
class MockAluRegisterFile()(implicit config: AluRegisterFileConfig)
  ...
  // The inputs to an ALU are registered, but there might be some extra logic
  // to handle write by-pass.
  alu.io.a := RegNext(registerFile.io.read(0).value)
  alu.io.b := RegNext(registerFile.io.read(1).value)
