...
def setup(chip):
    ...
    for tool in ('openroad', 'klayout', 'magic'):
        chip.set('pdk', process, 'aprtech', tool, stackup, libtype, 'lef',
                 os.path.abspath(os.path.join(pdkdir, 'lef', 'NangateOpenCellLibrary.tech.lef')))

    # Klayout setup file
    chip.set('pdk', process, 'layermap', 'klayout', 'def', 'gds', stackup,
             os.path.abspath(os.path.join(pdkdir, 'FreePDK45.lyt')))

    chip.set('pdk', process, 'display', 'klayout', stackup,
             os.path.abspath(os.path.join(pdkdir, 'FreePDK45.lyp')))
    ...
    chip.set('pdk', process, 'var', 'openroad', stackup, 'pad_global_place', ['0'])
