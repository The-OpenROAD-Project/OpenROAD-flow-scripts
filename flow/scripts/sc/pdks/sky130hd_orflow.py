...
def setup(chip):
    ...
    pdkdir = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'platforms', 'sky130hd')
    ...
    for tool in ('openroad', 'klayout', 'magic'):
        chip.set('pdk', process,'aprtech',tool,stackup, libtype,'lef',
                 os.path.abspath(os.path.join(pdkdir, 'lef', 'sky130_fd_sc_hd.tlef')))

    # Openroad specific files
    chip.set('pdk', process, 'aprtech','openroad', stackup, libtype,'tapcells',
             os.path.abspath(os.path.join(pdkdir, 'tapcell.tcl')))
    ...
    chip.set('pdk', process, 'layermap', 'klayout', 'def', 'gds', stackup,
             os.path.abspath(os.path.join(pdkdir, 'sky130hd.lyt')))
    chip.set('pdk', process, 'display', 'klayout', stackup,
             os.path.abspath(os.path.join(pdkdir, 'sky130hd.lyp')))
    ...
    chip.set('pdk', process, 'var', 'openroad', stackup, 'pad_global_place', ['1'])
    chip.set('pdk', process, 'var', 'openroad', stackup, 'pad_detail_place', ['0'])
    chip.set('pdk', process, 'var', 'openroad', stackup, 'macro_place_halo', ['40', '40'])
