...
def setup(chip):
    ...
    pdkdir = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'platforms', 'sky130hs')
    ...
    for tool in ('openroad', 'klayout', 'magic'):
        chip.set('pdk', process,'aprtech',tool,stackup, libtype,'lef',
                 os.path.abspath(os.path.join(pdkdir, 'lef', 'sky130_fd_sc_hs.tlef')))
    ...
    chip.set('pdk', process, 'layermap', 'klayout', 'def', 'gds', stackup,
             os.path.abspath(os.path.join(pdkdir, 'sky130hs.lyt')))
    chip.set('pdk', process, 'display', 'klayout', stackup,
             os.path.abspath(os.path.join(pdkdir, 'sky130hs.lyp')))
