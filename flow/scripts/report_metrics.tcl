...
proc report_metrics { stage when {include_erc true} {include_clock_skew true} } {
  ...
  if {$include_erc} {
    ...
    set reg_to_reg_critical_path [lindex [find_timing_paths -path_delay max -from [all_registers] -to [all_registers]] 0]
    if {$reg_to_reg_critical_path != ""} {
      set target_clock_latency_max [sta::format_time [$reg_to_reg_critical_path target_clk_delay] 4]
    } else {
      set target_clock_latency_max 0	
    }


    set reg_to_reg_critical_path [lindex [find_timing_paths -path_delay min -from [all_registers] -to [all_registers]] 0]
    if {$reg_to_reg_critical_path != ""} {
      set target_clock_latency_min [sta::format_time [$reg_to_reg_critical_path target_clk_delay] 4]
    } else {
