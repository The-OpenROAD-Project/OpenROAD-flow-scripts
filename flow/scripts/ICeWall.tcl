namespace eval ICeWall {
  ...
  proc normalize_locations {} {
    ...
    foreach side_name {"bottom" "right" "top" "left"} {
      ...
      dict for {inst_name inst} $side {
        set cell_type [dict get $library types [dict get $side $inst_name type]]
        ...
        set cell [$db findMaster $cell_name]
        set cell_width [$cell getWidth]
        set cell_height [$cell getHeight]

        # set location of padcell
        if {[dict exists $inst cell centre]} {
          set centre [list \
            x [expr round([dict get $inst cell centre x] * $units)] \
            y [expr round([dict get $inst cell centre y] * $units)] \
          ]

          dict set footprint padcells $side_name $inst_name cell scaled_origin [get_origin $centre $cell_width $cell_height [dict get $inst cell orient]]
          dict set footprint padcells $side_name $inst_name cell scaled_centre $centre
          
        } elseif {[dict exists $inst cell origin]} {
          # Ensure that we're working with database units
          set origin [list \
            set x [expr round([dict get $inst cell origin x] * $units)]
            set y [expr round[[dict get $inst cell origin y] * $units)]
          ]

          dict set footprint padcells $side_name $inst_name cell scaled_origin $origin
          dict set footprint padcells $side_name $inst_name cell scaled_centre [get_centre $origin $cell_width $cell_height [dict get $inst cell orient]]
        }
        ...
        if {[dict exists $inst bondpad centre]} {
          ...
          set centre [list \
            x [expr round([dict get $inst bondpad centre x] * $units)] \
            y [expr round([dict get $inst bondpad centre y] * $units)] \
          ]
          dict set footprint padcells $side_name $inst_name bondpad scaled_origin [get_origin $centre $bondpad_width $bondpad_height [dict get $inst bondpad orient]]
          dict set footprint padcells $side_name $inst_name bondpad scaled_centre $centre
        } elseif {[dict exists $inst bondpad origin]} {
          set x [expr round([dict get $inst bondpad origin x] * $units)]
          set y [expr round[[dict get $inst bondpad origin y] * $units)]

          dict set footprint padcells $side_name $inst_name bondpad scaled_origin $origin
          dict set footprint padcells $side_name $inst_name bondpad scaled_origin [get_centre $origin $bondpad_width $bondpad_height [dict get $inst cell orient]]
        }
      }
    }
  }
  ...
  proc get_cell_master {name} {
    ...
    foreach lib [$db getLibs] {
      if {[set cell [$lib findMaster $name]] != "NULL"} {return $cell}
    }
  }
  ...
  proc get_cells {type side} {
    ...
    foreach type_name [dict get $library types $type] {
      if {[dict exists $library cells $type_name cell_name]} {
        lappend cell_list [get_cell_master [dict get $library cells $type_name cell_name $side]]
      } else {
        lappend cell_list [get_cell_master $type_name]
      }
    }
    return $cell_list
  }
  ...
  proc fill_box {xmin ymin xmax ymax side} {
    ...
    foreach cell $filler_cells {
      dict set spacers [$cell getWidth] $cell
    }
    ...
    while {$fill_start < $fill_end} {
      ...
      if {$fill_start + $smallest_width >= $fill_end} {
        ...
        set spacer [dict get $spacers [lindex $spacer_widths end]]
      } else {
        # Find the largest spacer that will fit in the gap
        foreach space $spacer_widths {
          set width [[dict get $spacers $space] getWidth]
          if {[expr $fill_start + $width] <= $fill_end} {
            set spacer [dict get $spacers $space]
            break
          }
        }
      }

      set name "u_${type}_[dict get $idx $type]"
      set inst [odb::dbInst_create $block $spacer $name]
      switch $side \
        "bottom" {
          set x      $fill_start
          set y      $edge_bottom_offset
          set fill_start [expr $x + [$spacer getWidth]]
        } \
        "right"  {
          set x      [expr $chip_width - $edge_right_offset]
          set y      $fill_start
          set fill_start [expr $y + [$spacer getWidth]]
        } \
        "top"    {
          set x      [expr $fill_start + [$spacer getWidth]]
          set y      [expr $chip_height - $edge_top_offset]
          set fill_start $x
        } \
        "left"   {
          set x      $edge_left_offset
          set y      [expr $fill_start + [$spacer getWidth]]
          set fill_start $y
        }

      $inst setOrigin $x $y
      $inst setOrient [get_cell_orientation [$spacer getName] $side]
      $inst setPlacementStatus "FIRM"
