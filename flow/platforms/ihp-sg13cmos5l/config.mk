...
ifneq ($(FOOTPRINT_TCL),)
  ifeq ($(LOAD_ADDITIONAL_FILES),1)
    export ADDITIONAL_LEFS += $(PLATFORM_DIR)/lef/sg13cmos5l_io.lef
    export ADDITIONAL_SLOW_LIBS += $(PLATFORM_DIR)/lib/sg13cmos5l_io_slow_1p08V_3p0V_125C.lib
    export ADDITIONAL_FAST_LIBS += $(PLATFORM_DIR)/lib/sg13cmos5l_io_fast_1p32V_3p6V_m40C.lib
    export ADDITIONAL_TYP_LIBS += $(PLATFORM_DIR)/lib/sg13cmos5l_io_typ_1p2V_3p3V_25C.lib
    export ADDITIONAL_GDS += $(PLATFORM_DIR)/gds/sg13cmos5l_io.gds
  endif
endif
export TECH_LEF ?= $(PLATFORM_DIR)/lef/sg13cmos5l_tech.lef
export SC_LEF ?= $(PLATFORM_DIR)/lef/sg13cmos5l_stdcell.lef

export SLOW_LIB_FILES ?= $(PLATFORM_DIR)/lib/sg13cmos5l_stdcell_slow_1p08V_125C.lib \
                         $(ADDITIONAL_SLOW_LIBS)
export FAST_LIB_FILES ?= $(PLATFORM_DIR)/lib/sg13cmos5l_stdcell_fast_1p32V_m40C.lib \
                         $(ADDITIONAL_FAST_LIBS)
export TYP_LIB_FILES ?= $(PLATFORM_DIR)/lib/sg13cmos5l_stdcell_typ_1p20V_25C.lib \
                        $(ADDITIONAL_TYP_LIBS)
export LIB_FILES ?= $(TYP_LIB_FILES)
export GDS_FILES ?= $(PLATFORM_DIR)/gds/sg13cmos5l_stdcell.gds \
                    $(ADDITIONAL_GDS)
...
export TIEHI_CELL_AND_PORT = sg13cmos5l_tiehi L_HI
export TIELO_CELL_AND_PORT = sg13cmos5l_tielo L_LO

# Used in synthesis
export MIN_BUF_CELL_AND_PORTS = sg13cmos5l_buf_1 A X
...
export ABC_DRIVER_CELL = sg13cmos5l_buf_4
...
export KLAYOUT_TECH_FILE = $(PLATFORM_DIR)/sg13cmos5l.lyt
#
...
# DRC Check
export KLAYOUT_DRC_FILE ?= $(PLATFORM_DIR)/drc/sg13cmos5l_minimal.lydrc

#LVS Check
export CDL_FILE ?= $(PLATFORM_DIR)/cdl/sg13cmos5l_stdcell.cdl
#export KLAYOUT_LVS_FILE = $(PLATFORM_DIR)/lvs/$(PLATFORM).lylvs

export REMOVE_CELLS_FOR_LEC ?= "bondpad_70* sg13cmos5l*"
